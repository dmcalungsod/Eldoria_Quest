...
def insert_monsters(conn):
    ...
    for m in monsters.MONSTERS.values():
        data.append((
            m.get("name"), m.get("description", ""), m.get("tier", "Normal"),
            int(m.get("level", 1)), int(m.get("hp", 1)), int(m.get("atk", 1)),
            int(m.get("def", 0)), int(m.get("dex", 0)), int(m.get("magic", 0)),
            int(m.get("xp", 1)), m.get("biome", "Forest")
        ))
...
def insert_quest_items(conn):
    ...
    data = [(q["name"], q.get("notes", ""), q.get("rarity", "Common"), q.get("value", 0)) for q in quest_items.QUEST_ITEMS.values()]
    conn.executemany(
        "INSERT INTO quest_items (name, description, rarity, quest_id, value) VALUES (?, ?, ?, NULL, ?)",
        data
    )
    conn.commit()
...
def insert_consumables(conn):
    ...
    for c in consumables.CONSUMABLES.values():
        data.append((
            c["name"], c.get("description", ""), c.get("rarity", "Common"),
            json.dumps(c.get("effect", {})), c.get("value", 0)
        ))

    conn.executemany(
        "INSERT INTO consumables (name, description, rarity, effect, value) VALUES (?, ?, ?, ?, ?)",
        data
    )
    conn.commit()
...
def insert_equipments(conn):
    ...
    for e in EQUIPMENT_DATA.values():
        s = e.get("stats_bonus", {})
        data.append((
            e["name"], e.get("description", ""), e.get("rarity", "Common"), e.get("slot", "accessory"),
            s.get("STR", 0), s.get("END", 0), s.get("DEX", 0), s.get("AGI", 0), s.get("MAG", 0), s.get("LCK", 0),
            e.get("level_req", 1)
        ))
...
def insert_class_equipments(conn):
    ...
    for ce in CLASS_EQUIPMENTS.values():
        ...
        data.append((
            cid, ce["name"], ce.get("description", ""), ce.get("rarity", "Common"), ce.get("slot", "accessory"),
            s.get("STR", 0), s.get("END", 0), s.get("DEX", 0), s.get("AGI", 0), s.get("MAG", 0), s.get("LCK", 0),
            ce.get("set"), ce.get("level_req", 1)
        ))
...
def insert_materials(conn):
    ...
    conn.executemany(
        "INSERT OR IGNORE INTO materials (key_id, name, description, rarity, value) VALUES (?, ?, ?, ?, ?)",
        data
    )
    conn.commit()
