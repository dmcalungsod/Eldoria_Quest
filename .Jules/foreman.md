...
- **2026-03-13:** Discovered via Analyst that overlooking entity integration when designing new regions can result in severe progression drops (-96% EV in The Undergrove). Always ensure the required database elements (monsters, materials, descriptions) are correctly implemented in game_systems/data schemas. Missing schema entities have an immense impact on the economy.
