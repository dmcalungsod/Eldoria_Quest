...
async def back_to_profile_callback(interaction: discord.Interaction, is_new_message: bool = False):
    ...
    try:
        ...
        if rank in ["F", "E"]:
            try:
                equipped_items = await asyncio.to_thread(db.get_equipped_items, discord_id)
                weapon_slots = {
                    "sword",
                    "mace",
                    "wand",
                ...
                exp_stats = await asyncio.to_thread(db.get_exploration_stats, discord_id)
                total_expeditions = exp_stats.get("total_expeditions", 0) if exp_stats else 0

                active_quests = await asyncio.to_thread(db.get_player_quests_joined, discord_id)
                ...
                if advice:
                    embed.add_field(name="🎓 Mentor's Advice", value=f"*{advice}*", inline=False)
