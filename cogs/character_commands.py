...
class CharacterCommands(commands.Cog):
    ...
    @app_commands.command(name="status", description="View your Falna (Adventurer Status).")
    async def status(self, interaction: discord.Interaction):
        ...
        if not player:
            await interaction.followup.send("You have not registered with the Guild yet. Use `/start`.", ephemeral=True)
            return

        # Get Guild Rank
        conn = self.db.connect()
        cur = conn.cursor()
        cur.execute("SELECT rank, merit_points FROM guild_members WHERE discord_id = ?", (discord_id,))
        ...
        # Rank & Level
        embed.add_field(name="Condition", value=f"**Lv.** {player['level']}\n**Rank:** {guild_data['rank']}", inline=True)
        embed.add_field(name="Progress", value=f"**EXP:** {player['experience']} / {player['exp_to_next']}\n**Merit:** {guild_data['merit_points']}", inline=True)
        ...
        # Resources
        embed.add_field(name="Vitals", value=f"{E.HP} **HP:** {stats.max_hp}\n{E.MP} **MP:** {stats.max_mp}", inline=True)
        embed.add_field(name="Wealth", value=f"{E.GOLD} **Valis:** {player['gold']}", inline=True)
    ...
    async def inventory(self, interaction: discord.Interaction):
        ...
        if not items:
            await interaction.response.send_message(f"{E.BACKPACK} Your backpack is empty.", ephemeral=True)
            return
