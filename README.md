...
<p align="center">
  ...
  <img src="https://img.shields.io/github/last-commit/dmcalungsod/Eldoria_Quest?style=flat" alt="Last Commit"/>
  <img src="https://img.shields.io/github/commit-activity/y/dmcalungsod/Eldoria_Quest?style=flat" alt="Commit Activity"/>
</p>
...
Eldoria Quest utilizes GitHub Actions to ensure code quality, security, and smooth integration. The repository includes several automated workflows:

### 🛠️ Continuous Integration (CI)

Runs automatically on every push to `main` and all Pull Requests. It ensures the codebase remains stable and secure through:

- **Static Analysis & Formatting:** `Ruff` is used to strictly enforce code style and catch linting errors. It also automatically formats the code before running checks.
- **Security Scanning:** `Pip-Audit` checks dependencies for known vulnerabilities, and `Bandit` scans the Python code for common security flaws.
- **Automated Testing:** The `Pytest` suite validates game mechanics and outputs a code coverage report.
- **Coverage Tracking:** Test coverage is automatically uploaded and tracked via Codacy.
- **Repository Mirroring:** Any changes merged into `main` are automatically mirrored to a hidden backup repository to ensure data redundancy.

### 🔄 Pull Request Automation

- **Auto-Merge:** Approved Pull Requests are automatically squashed and merged into `main` to streamline the development flow.
- **PR Assistant:** Automated comments remind contributors to verify timestamps and logging rules upon creating a new Pull Request.
