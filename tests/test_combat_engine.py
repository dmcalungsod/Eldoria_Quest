...
def mock_player():
    ...
    player.stats.get_total_stats_dict.return_value = {
        "STR": 10, "END": 10, "DEX": 10, "AGI": 10, "MAG": 10, "LCK": 10, "HP": 100, "MP": 50
    }
...
def test_recoil_damage(mock_player, mock_monster):
    ...
    with patch("game_systems.combat.combat_engine.DamageFormula.player_skill") as mock_calc:
        ...
        with patch.object(engine, "_decide_player_skill", return_value={"skill": skill, "reason": "Test"}):
...
def test_status_immunity(mock_player, mock_monster):
    ...
    with patch("game_systems.monsters.monster_actions.MonsterAI.choose_action", return_value={"type": "skill", "skill": monster_skill}):
        # Force player to miss/do nothing to ensure monster turn happens
        with patch("game_systems.combat.combat_engine.DamageFormula.player_attack", return_value=(0, False, "miss")):
            ...
            assert engine.player_stunned == False
...
def test_negative_stat_debuff(mock_player, mock_monster):
    ...
    with patch.object(engine, "_decide_player_skill", return_value={"skill": skill, "reason": "Test"}):
        ...
        monster_data = result.get("monster_data", result.get("monster_hp")) # Check returned state
