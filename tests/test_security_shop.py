...
@pytest.mark.asyncio
async def test_shop_callback_validation_success():
    ...
    with patch(
        "cogs.shop_cog.get_player_or_error", new=AsyncMock(return_value={"aurum": 1000})
    ):
...
@pytest.mark.asyncio
async def test_shop_callback_validation_failure_empty():
...
@pytest.mark.asyncio
async def test_shop_callback_validation_failure_malformed():
