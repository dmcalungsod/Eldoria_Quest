...
class TestAbyssalTideEvent(unittest.TestCase):
    def setUp(self):
    ...
    def test_threat_reduction_in_sunken_grotto(self, mock_locations):
        ...
        self.assertEqual(passed_threat_reduction, 0.9, "Threat reduction was not applied correctly in Sunken Grotto.")

    @patch("game_systems.adventure.adventure_session.LOCATIONS")
    def test_threat_reduction_not_applied_elsewhere(self, mock_locations):
        """Test that the threat reduction is NOT applied outside the Sunken Grotto."""
        ...
        self.assertEqual(
            passed_threat_reduction, 1.0, "Threat reduction was incorrectly applied outside Sunken Grotto."
        )

    @patch("game_systems.adventure.adventure_rewards.LootCalculator")
    def test_loot_bonus_in_sunken_grotto(self, mock_loot_calc):
        ...
        # Expected: 1.25 (global) * 1.5 (local Sunken Grotto) = 1.875
        self.assertAlmostEqual(
            passed_loot_boost, 1.875, places=3, msg="Loot boost was not calculated correctly in Sunken Grotto."
        )

    @patch("game_systems.adventure.adventure_rewards.LootCalculator")
    def test_loot_bonus_not_applied_elsewhere(self, mock_loot_calc):
        """Test that the Sunken Grotto loot bonus is NOT applied elsewhere."""
        mock_loot_calc.roll_drops.return_value = ["test_item"]
        self.db_mock.get_player_stats_json.return_value = {"LUCK": {"base": 10}}
        self.db_mock.get_active_world_event.return_value = {"type": WorldEventSystem.ABYSSAL_TIDE}
        ...
        self.assertAlmostEqual(
            passed_loot_boost, 1.25, places=3, msg="Loot boost was incorrectly calculated outside Sunken Grotto."
        )
