...
class TestMonsterDataIntegrity(unittest.TestCase):
    def setUp(self):
        ...
        self.monsters_patcher = patch.dict("game_systems.data.monsters.MONSTERS", {self.test_monster_key: self.test_monster_data}, clear=True)
    ...
    def test_spectral_tide_does_not_mutate_global_monsters(self, MockWorldTime, MockWorldEventSystem):
        ...
        with patch("random.choices", return_value=[self.test_monster_key]), \
             patch("random.random", side_effect=[0.1, 0.9]): # 0.1 triggers Spectral, 0.9 fails next time
            ...
            self.assertFalse(has_ecto,
                             "CRITICAL: Global MONSTERS data was modified! 'ectoplasm' found in template.")
            ...
            self.assertFalse(any(d[0] == "ectoplasm" for d in monster2["drops"]),
