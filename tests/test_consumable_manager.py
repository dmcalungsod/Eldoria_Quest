...
def test_triage_potency(manager, mock_db):
    ...
    mock_db.get_inventory_item.return_value = {
        "item_key": "hp_potion_1", "item_type": "consumable", "count": 1
    }

    # Mock Player Data (Base stats return HP as 50 + 10*10 = 150)
    mock_db.get_player_vitals.return_value = {"current_hp": 50, "current_mp": 50}
    mock_db.get_player_stats_json.return_value = {
        "HP": 150, "MP": 100, "STR": 10, "END": 10, "DEX": 10, "AGI": 10, "MAG": 10, "LCK": 10
    }
    ...
    with patch("game_systems.items.consumable_manager.CONSUMABLES", {"hp_potion_1": {"effect": {"heal": 50}, "name": "Potion", "type": "hp"}}):
        with patch("game_systems.items.consumable_manager.SKILLS", {"triage": {"type": "Passive", "passive_bonus": {"healing_item_potency": 0.2}}}):
            mock_db.consume_item_atomic.return_value = True

            # Need to patch PlayerStats.from_dict because it calculates max_hp internally based on END
...
def test_triage_potency_uncapped(manager, mock_db):
    ...
    mock_db.get_player_vitals.return_value = {"current_hp": 10, "current_mp": 50}
    mock_db.get_player_skill_levels.return_value = {"triage": 1}
