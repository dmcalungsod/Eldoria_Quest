...
def test_builders_boon_gathering_multiplier(mock_db, mock_quest_system):
    ...
    with patch("random.randint", return_value=1), patch(
        "random.choices", return_value=["ancient_wood"]
    ):
        with patch(
            "game_systems.data.materials.MATERIALS",
            {"ancient_wood": {"name": "Ancient Wood"}},
        ):
            result = event_handler._perform_wild_gathering(
                mock_context, location_id="test_loc"
            )
...
def test_builders_boon_gathering_multiplier_ignores_other_items(
    mock_db, mock_quest_system
):
    ...
    with patch("random.randint", return_value=1), patch(
        "random.choices", return_value=["medicinal_herb"]
    ):
        with patch(
            "game_systems.data.materials.MATERIALS",
            {"medicinal_herb": {"name": "Medicinal Herb"}},
        ):
            result = event_handler._perform_wild_gathering(
                mock_context, location_id="test_loc"
            )
