...
def main():
    ...
    print(
        f"{'Location Name':<30} | {'Rank':<4} | {'Lvl':<3} | {'Kill EV':<8} | {'Gather EV':<9} | {'Est EV/Trip (30m)'}"
    )
    print("-" * 80)
    ...
    for res in results:
        print(
            f"{res['name']:<30} | {res['rank']:<4} | {res['level']:<3} | {res['kill_ev']:>8.2f} | {res['gather_ev']:>9.2f} | {res['est_trip_ev']:>8.2f}"
        )
