...
class QuestsMenuView(View, GuildViewMixin):
    def __init__(self, db_manager: DatabaseManager, interaction_user: discord.User):
        ...
        # Buttons
        self.add_item(ViewFactory.create_button("Quest Board", discord.ButtonStyle.primary, "g_q_board", E.SCROLL, 0, callback=self.view_quests_callback))
        self.add_item(ViewFactory.create_button("Quest Ledger", discord.ButtonStyle.primary, "g_q_ledger", E.QUEST_SCROLL, 0, callback=self.view_quest_ledger_callback))
        self.add_item(ViewFactory.create_button("Quest Turn-In", discord.ButtonStyle.success, "g_turn_in", E.MEDAL, 0, callback=self.quest_turn_in_callback))
        self.add_item(ViewFactory.create_button("Check Rank", discord.ButtonStyle.secondary, "g_check_rank", "🏅", 1, callback=self.check_rank_callback))
        
        self.back_btn = ViewFactory.create_button("Back to Guild Lobby", discord.ButtonStyle.grey, "back_lobby", row=1, callback=back_to_guild_hall_callback)
    ...
    async def view_quests_callback(self, interaction: discord.Interaction, button: Button = None):
        ...
        embed = discord.Embed(title=f"{E.SCROLL} Quest Board", description="Select a contract.", color=discord.Color.dark_green())
        embed.add_field(name="Available Contracts", value="Choose a quest from the dropdown.", inline=False)
    ...
    async def view_quest_ledger_callback(self, interaction: discord.Interaction, button: Button = None):
        ...
        embed = discord.Embed(title=f"{E.QUEST_SCROLL} Quest Ledger", description="Your active contracts.", color=discord.Color.from_rgb(139, 69, 19))
        
        if not quests:
            embed.add_field(name="No Active Contracts", value="Visit the Quest Board.")
        else:
            for q in quests:
                progress = self._format_progress(q)
                embed.add_field(name=f"{q['title']}", value="\n".join(progress) or "No objectives.", inline=False)
    ...
    async def quest_turn_in_callback(self, interaction: discord.Interaction, button: Button = None):
        ...
        embed = discord.Embed(title=f"{E.MEDAL} Quest Turn-In", description="Select a quest to report.", color=discord.Color.gold())
        if not quests:
            embed.add_field(name="No Active Contracts", value="Nothing to turn in.")
    ...
    async def check_rank_callback(self, interaction: discord.Interaction, button: Button = None):
        ...
        if not next_rank:
            embed = discord.Embed(title="Rank Status", description="Max Rank Reached.", color=discord.Color.gold())
            ...
            for k, v in reqs.items():
                curr = data.get(k, 0)
                lines.append(f"• {k.replace('_',' ').title()}: {curr}/{v}")
                if curr < v: eligible = False
            
            embed = discord.Embed(title=f"Promotion: {cur_rank} -> {next_rank}", color=discord.Color.blue())
    ...
    def _format_progress(quest):
        ...
        for type, tasks in objs.items():
            if isinstance(tasks, dict):
                for t, req in tasks.items():
                    cur = prog.get(type, {}).get(t, 0)
                    lines.append(f"• {t}: {cur}/{req}")
            else:
                cur = prog.get(type, {}).get(tasks, 0)
                lines.append(f"• {type.title()} {tasks}: {cur}/1")
        return lines