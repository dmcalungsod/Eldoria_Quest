    ...
    def validate(data: Any, schema: Dict[str, Any], path: str = "") -> List[str]:
        ...
        if expected_type:
            if not isinstance(data, expected_type):
                # Handle generic types like List, Dict if imported from typing, but we use built-in list, dict in schemas
        ...
        if isinstance(data, list):
            ...
            if "elements" in schema:
                if len(data) != len(schema["elements"]):
                    # Already checked length if specified, but safety check
                    pass
                else:
