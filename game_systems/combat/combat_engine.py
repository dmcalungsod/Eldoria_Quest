...
class CombatEngine:
    ...
    def _process_monster_turn(self, log, turn_report, player_defending, monster_stunned) -> bool:
        ...
        if action["type"] == "attack":
            self._monster_action_attack(effective_monster, player_defending, turn_report, log)

        elif action["type"] == "skill":
            self._monster_action_skill(action["skill"], effective_monster, player_defending, turn_report, log)
            ...
            self._monster_action_execute_charge(action["skill"], effective_monster, player_defending, turn_report, log)

        return self.player_hp <= 0

    def _monster_action_attack(self, effective_monster, player_defending, turn_report, log):
    ...
    def _monster_action_skill(self, skill, effective_monster, player_defending, turn_report, log):
    ...
    def _monster_action_execute_charge(self, skill, effective_monster, player_defending, turn_report, log):
    ...
    def _execute_player_skill(self, skill, log, turn_report, force_crit=False):
        ...
        if skill.get("heal_power", 0) > 0:
            ...
            damage_dealt = self._handle_damage_skill(skill, skill_level, log, turn_report, force_crit)
    ...
    def _handle_damage_skill(self, skill, skill_level, log, turn_report, force_crit=False) -> int:
        effective_monster = self._get_effective_monster_stats()
    ...
    def _prioritize_and_select_skill(self, usable_skills: list) -> dict:
        ...
        offensive_skills = [
            s for s in usable_skills if s.get("heal_power", 0) == 0 and not s.get("buff", s.get("buff_data"))
        ]
        if offensive_skills:
