...
class ItemManager:
        ...
        try:
            with self.connect() as conn:
                ...
                return {
                    ...
                    if key in row.keys() and row[key] != 0
                }
    ...
    # GENERIC ITEM LOOKUP
    # --------------------------------------------------------------------
    def _lookup(self, table: str, query: str, params: tuple) -> Optional[sqlite3.Row]:
    ...
    def get_class_equipment_for_class(self, class_id: int, level: int = 1):
        try:
            with self.connect() as conn:
                return conn.execute(
                    "SELECT * FROM class_equipment WHERE class_id = ? AND min_level <= ?", 
                    (class_id, level)
                ).fetchall()
        except Exception as e:
    ...
    def format_consumable_effect(self, effect_json):
        if not effect_json: return "No effect"
        try:
